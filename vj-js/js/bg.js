...
const backgroundImage = {
	...
	applyRandomEffect(duration, target) {
		...
		const effectTween = TweenMax.to($target, time, {
			css: {
				filter: `blur(${util.randomRange(0, 1)}px) brightness($${util.randomRange(50, 150)}%) contrast(${util.randomRange(50, 150)}%) grayscale(${util.randomRange(0, 25)}%) hue-rotate(${util.randomRange(0, 360)}deg) invert(${util.randomDoubleRange(0, 15, 85, 100)}%) saturate(${util.randomRange(50, 150)}%) sepia(${util.randomRange(0, 40)}%)`
			},
			onComplete: cancelAnimation
		});
...
const controls = {
	...
	initLoop() {
		return setInterval(() => {
			...
			backgroundImage.changeBackgroundImage(target, util.randomRange(1, backgroundImage.data.amount));
